...
- **Testing**: domain-heavy unit tests for ledger/merge/promotion invariants; integration tests against real PostgreSQL (testcontainers); the ADR-0031 Stitch Interaction Budget runs on reference devices as a release gate (see the [release checklist](release-checklist.md) for details), including worst cases — fully completed 300×300 Pattern, rapid stitching during pan/zoom, memory pressure, GPU-context loss, and app resume.
