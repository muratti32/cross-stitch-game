...
| Apple Search Ads attribution and ROAS | RevenueCat Apple AdServices + Apple Ads Campaign Management API | **Partially implemented** — iOS token collection and local cohort calculator are in the repo; dashboard authorization, spend credentials, and export ingestion remain pending | RevenueCat supplies Apple Search Ads attribution and net proceeds; Apple Ads supplies spend. Match by campaign, ad group, and keyword IDs and report 7/30/90-day cohorts. | See [`docs/apple-ads-roas.md`](apple-ads-roas.md); no Apple Ads credential or secret value is stored in the repo |
