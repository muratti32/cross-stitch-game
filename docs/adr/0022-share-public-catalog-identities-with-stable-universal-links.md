...
## Amendment: Temporary backend failure is not unavailability

An unavailable page asserts that the target is under Review Hold, Account Closure Hold, Catalog Withdrawal, Safety Removal, or no longer exists. A Catalog Share Link landing page that cannot reach the Game Backend (network error, 5xx response, or a structurally invalid body) cannot establish that, so it returns the metadata-free deep-link redirect page with HTTP 503, `Cache-Control: no-store`, and `Retry-After` instead of the unavailable page. Crawlers and link previews then treat the failure as temporary rather than caching a removal, and no hidden content is exposed because the page carries no catalog or profile metadata. Non-5xx error responses still resolve to the unavailable page (#253).
