...
> The Remaining Cell Locator pricing rule in this ADR is superseded by ADR-0058. Its accessibility, numbering, and non-auto-fill rules remain in force.
...
Remaining Cell Locator is an online-gated, 1-Stitch-Coin action after a successful server-authoritative Locator Attempt. It deterministically centers the next unfinished cell of Active Thread Color but does not fill it, select another color, change progress, grant rewards, or show advertising; no-target, failure, cancellation, and offline use charge nothing. The first release deliberately omits auto-fill. Mismatched Tap remains a gentle, non-punitive no-op, and Undo remains free while the session is active.
