...
export class AiArtworkService {
  ...
  private async accountStatus(p: AuthPrincipal): Promise<'active' | 'closing' | 'closed' | 'deleted' | null> { return p.type === PrincipalType.Account ? this.accountStateService.getAccountStatus(p.id) : null; }
