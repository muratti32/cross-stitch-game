...
import type { ConversionProfile } from '@/conversion';

export type ArtworkAspect = 'square' | 'portrait_4_3' | 'landscape_4_3';
export interface AiArtwork { id: string; aspect: ArtworkAspect; status: string; failureReason: string | null; createdAt: string; imageUrl: string | null; }
...
export async function generateAiArtwork(prompt: string, aspect: ArtworkAspect) { const r = await apiFetch('/v1/ai-artworks', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt, aspect }) }); if (!r.ok) throw new Error((await message(r)) ?? `Generation request failed (${r.status})`); return r.json() as Promise<{ id: string }>; }
